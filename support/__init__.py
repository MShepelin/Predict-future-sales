...
def get_prices_means(dataset: pd.DataFrame):
    item_mean_prices = dataset.groupby('item_id')['item_price'].mean().fillna(0).astype('int32')
    item_mean_prices.name = 'item_mean_prices'
    shop_mean_prices = dataset.groupby('shop_id')['item_price'].mean().fillna(0).astype('int32')
    shop_mean_prices.name = 'shop_mean_prices'
    cat_mean_prices = dataset.groupby('item_category_id')['item_price'].mean().fillna(0).astype('int32')
...
def set_categories_features(full, train_mask, category_columns):
    ...
    # One Hot
    full = full.join(pd.get_dummies(full['item_category_id'], prefix='category'))
